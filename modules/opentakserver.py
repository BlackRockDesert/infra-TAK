...
OTS_TAG = "1.7.13"  # pinned release tag (no 'v' prefix)
...
services:
  ...
  opentakserver:
    ...
    environment:
      ...
      - DOCKER_OTS_RABBITMQ_USERNAME=ots
      - DOCKER_OTS_RABBITMQ_PASSWORD={rabbit_password}
...
'''
        ...
        password_salt = s.get('ots_password_salt') or ''
        if not password_salt or password_salt.isdigit():
            password_salt = 'ots_' + str(_sec.randbits(128))
        ...
        # CoT streaming (CloudTAK/EUDs connect here) + certificate enrollment + HTTPS API
        # API (8081) is loopback-only — Caddy proxies from 443
